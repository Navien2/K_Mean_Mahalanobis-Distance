...
public class KMeanChart extends ApplicationFrame {
    ...
    public KMeanChart(String title, double[] x, double[] y, double[] x1, double[] y1, double[] x2, double[] y2) {
        super(title);
     ...
     * @return A panel.
     */
    public JPanel createDemoPanel(double[] x, double[] y, double[] x1, double[] y1, double[] x2, double[] y2) {
        XYDataset dataset = new SampleXYDataset2(x, y);
        JFreeChart chart = ChartFactory.createScatterPlot("Scatter Plot",
                "X", "Y", dataset, PlotOrientation.VERTICAL, true, true, false);
    ...
    public JPanel createDemoPanel(double[][] xs, double[][] ys, int k, double[][] centroids) {
        ...
        for (int i = 1; i < xs.length; i++) {
            ...
            plot.getRendererForDataset(plot.getDataset(i)).setSeriesPaint(0, Color.getHSBColor((float) i / (float) k, 1.0F, 1.0F));
